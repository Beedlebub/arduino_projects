...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    if ((char)payload[0] == '1') {
      ...
      writeEasyNeoPixel(0, (char)payload[i]);
    }
  }
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      /*  Try subscribing to 3 more topics; red, green, and blue.
          Set up a single numeric input field for each to start.
          Not sure how to pass after all have been changed, 
